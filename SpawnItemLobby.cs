...
{
    public class SpawnItemLobby : MonoBehaviour
    {
        private static SpawnItemLobby _instance;
        private static SpawnItemLobby Instance
        {
            get
            {
                if (_instance == null)
                {
                    GameObject go = new GameObject("SpawnItemLobby_Instance");
                    DontDestroyOnLoad(go);
                    _instance = go.AddComponent<SpawnItemLobby>();
                }
                return _instance;
            }
        }
        ...
        private static Interactable SpawnItemOnDoormat(NewAddress address, InteractablePreset itemPreset, Human owner, string itemNameForLog)
        {
            ...
            if (doormatSubObjects.Count > 0)
            {
                ...
                float rightOffset = 1.0f;     // X - to the side
